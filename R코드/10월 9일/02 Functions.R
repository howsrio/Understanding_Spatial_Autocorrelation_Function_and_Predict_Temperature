...
NegLogL_optim2=function(log_paras){
  ...
  a=round(exp(log_paras[1]), 15)  #sill
  b=round(exp(log_paras[2]), 15)  #spatial range
  nu=round(exp(log_paras[2]), 15) #smoothness parameter
  x=chordal_matrix 
...
MLE_test2=function(train_data, test_data){
  ...
  log_paras=round(log(abs(parameters)+0.0000001), 8)
