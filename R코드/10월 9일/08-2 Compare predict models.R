...
write.xlsx(mean.df, file="Compare models result for normal data.xlsx",overwite=FALSE)
write.xlsx(all.df, file="Compare models All result for normal data.xlsx",overwite=FALSE)
...
for (count in 1:16){
  ...
  Compare_models_trimed_mean[count+1,] = 
    apply(All_result[(1+count*12):(10+count*12),], 2, function(x) trimmean(x, proportion = 0.2, na.rm = TRUE))
}
