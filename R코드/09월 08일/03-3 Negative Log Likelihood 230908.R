...
#∎Question∎. 왜 method는 'L-BFGS-B'로 정한거지?
#Answer : ChatGPT 왈, likelihood는 'L-BFGS-B'로 계산
#∎Question∎. 각 method들의 차이점을 좀 알아야할듯
#(20230904) 왜 하필 L-BFGS-B로 결정했는지에 대한 더 명확한 이유가 필요할듯함.

#----------------------------------------------------------------------------------
#optim의 결과
#summer17_MLE=c(1.0146339,180.9073149,0.1837886) #Save ML estimators
#summer17_NLL=336.8109 #Compute negative log likelihood
...
subMat_11=Summer22_tatol_CovMat[1:nrow(rem_point),1:nrow(rem_point)]
subMat_12=Summer22_tatol_CovMat[1:nrow(rem_point),(nrow(rem_point)+1):nrow(total_point)]
subMat_21=Summer22_tatol_CovMat[(nrow(rem_point)+1):nrow(total_point),1:nrow(rem_point)]
subMat_22=Summer22_tatol_CovMat[(nrow(rem_point)+1):nrow(total_point),(nrow(rem_point)+1):nrow(total_point)]
